...
class geolocation
{
  ...
  */
  protected function _query() {
    ...
    if (!($d = file_get_contents2("http://" . $this->_apiDomain . "/$api?ip={$this->_ips[0]}&timezone=$showTimezone"))) {
      $this->_setError(new Exception(self::CONNECT_ERROR));
      //Try backup server
      if (!($d = file_get_contents2("http://" . $this->_apiBackupDomain . "/$api?ip={$this->_ips[0]}&timezone=$showTimezone"))) {
